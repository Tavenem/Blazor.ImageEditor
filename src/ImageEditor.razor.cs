...
public partial class ImageEditor
{
    ...
    {
        if (double.TryParse(e.Value as string, out var value))
        {
    ...
    {
        if (int.TryParse(e.Value as string, out var v)
            && Enum.IsDefined((DrawingMode)v))
        {
            await SetDrawingModeAsync((DrawingMode)v).ConfigureAwait(false);
        }
    }
    ...
    {
        if (int.TryParse(e.Value as string, out var v)
            && Enum.IsDefined((StrokeLineCap)v))
        {
            await SetLineCapAsync((StrokeLineCap)v).ConfigureAwait(false);
        }
    }
